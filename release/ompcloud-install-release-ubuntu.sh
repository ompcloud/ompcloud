...
export OMPCLOUD_INSTALL_PREFIX="$(pwd)"
export MAKE_ARGS="-j4"

export CGCLOUD_HOME="$OMPCLOUD_INSTALL_PREFIX/cgcloud"
#export LIBHDFS3_SRC="$OMPCLOUD_INSTALL_PREFIX/libhdfs3"
export LIBHDFS3_BUILD="$OMPCLOUD_INSTALL_PREFIX"

export OMPCLOUD_CONF_DIR="$OMPCLOUD_INSTALL_PREFIX/conf"
export OMPCLOUD_SCRIPT_DIR="$OMPCLOUD_INSTALL_PREFIX/script"
...
export LLVM_BUILD="$OMPCLOUD_INSTALL_PREFIX"
#export LIBOMPTARGET_SRC="$OMPCLOUD_INSTALL_PREFIX"
export LIBOMPTARGET_BUILD="$OMPCLOUD_INSTALL_PREFIX"
...
export PATH="$PATH:$HADOOP_HOME/bin:$HADOOP_HOME/sbin:$SPARK_HOME/bin:$OMPCLOUD_INSTALL_PREFIX/bin"
...
# Files for org.llvm.openmp (sbt)
cp -R $OMPCLOUD_INSTALL_PREFIX/local $HOME/.ivy2/
...
cp $OMPCLOUD_INSTALL_PREFIX/conf-hdfs/core-site.xml $HADOOP_CONF
cp $OMPCLOUD_INSTALL_PREFIX/conf-hdfs/hdfs-site.xml $HADOOP_CONF
...
chmod ugo+x /usr/bin/hdfs-start /usr/bin/hdfs-stop /usr/bin/hdfs-reset
...
cmake $UNIBENCH_SRC -DOpenMP_CXX_FLAGS=-fopenmp=libomp -DOpenMP_C_FLAGS=-fopenmp=libomp -DCMAKE_BUILD_TYPE=Release  -DRUN_TEST=OFF -DRUN_BENCHMARK=ON -DCMAKE_BUILD_TYPE=Release
make experiments
